...
@Component({
  selector: 'app-room-where-am-i',
  templateUrl: './room-where-am-i.modal.html',
  styleUrls: ['./room-where-am-i.modal.scss'],
})
export class RoomWhereAmIModal {
      ...
      for (const level in this.data) {
        if (this.data[level]) {
          ...
          for (const location of locations) {
            if (
              location.exhibitor.name.toLowerCase().indexOf(searchValue.toLowerCase()) >= 0 ||
              location.numeroStand.toLowerCase().indexOf(searchValue.toLowerCase()) >= 0
            ) {
              newItems.push({
                level,
                location,
