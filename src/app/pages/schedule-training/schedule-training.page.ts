...
import { resetProgrammeScientificFilters, resetProgrammeTrainingFilters } from 'src/app/core/utils/reset-filters.utils';
...
export class ScheduleTrainingPage {
  ...
  private filtersConfigScientific: { [key: string]: Filter[] } = {};
  ...
  public init(strapi: StrapiStateInterface, showToast: boolean = true) {
    ...
    this.filterDays = cloneDeep(strapi.programme.filters.training.days);
    this.filtersConfig = cloneDeep(strapi.programme.filters.training);
    this.filtersConfigScientific = cloneDeep(strapi.programme.filters.scientific);

    const params = this.activatedRoute.snapshot.queryParams;
    if (params.queryText) {
      resetProgrammeTrainingFilters(this.filtersConfig);
      this.dataConfig.queryText.programmeTraining = decodeURIComponent(params.queryText);
    }
    ...
    this.sessionsStrapi = cloneDeep(strapi.programme.schedule.training);
      ...
      componentProps: {
        ...
        config: [
          ...
          },
          {
            ...
            icon: { src: '/assets/icon/icon_sponsor.svg' },
          },
          {
            val: 'sponsored',
            background: '/assets/img/filters/sponsor.png',
  ...
  public deleteFilter(filter: string | number, list: string): void {
    ...
    this.store.dispatch(setProgrammeFiltersTraining({ filters: this.filtersConfig }));
  ...
  public async updateSchedule(showToast: boolean): Promise<void> {
    ...
    const queryText = this.dataConfig.queryText.programmeTraining.toLowerCase().replace(/,|\.|-/g, ' ');
    this.sessionsToShow = [];
  ...
  public clickNews(): void {
    resetProgrammeScientificFilters(this.filtersConfigScientific);
    this.filtersConfigScientific?.tracks.forEach((v) => {
      if (v.val === 'Session paramédicale') {
        v.isChecked = true;
      }
    });

    this.store.dispatch(setProgrammeFiltersScientific({ filters: this.filtersConfigScientific }));
